...
4. Rebatir Objeciones: Si el cliente no llega con la plata, atacás: "No te hagas problema fiera, pasame qué auto tenés, te lo tomo en parte de pago a un número que no te da nadie en la calle y te llevás la chata hoy".
...
${VEHICLES.map(v => `- ${v.brand} ${v.model} ${v.version} (${v.year}) · ${v.body_type} · ${v.condition} · $${v.precio_ars.toLocaleString('es-AR')} ARS · ID: ${v.id}`).join('\n')}
